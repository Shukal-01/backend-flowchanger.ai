...
async function createFlexibleShift(req, res) {
  try {
    ...
    const flexibleShiftResult = FlexibleShiftSchema.safeParse({
      weekOff,
      staffId,
      shifts,
      dateTime,
    });
    console.log(flexibleShiftResult);

    const flexibleShift = await prisma.flexibleShift.create({
      data: {
        dateTime: new Date(flexibleShiftResult.data.dateTime),
        weekOff: flexibleShiftResult.data.weekOff,
        staffId: flexibleShiftResult.data.staffId,
        shifts: {
          connect: flexibleShiftResult?.data?.shifts?.map((id) => ({ id })),
        },
      },
    });
