...
const getCustomFieldById = async (req, res) => {
    ...
    try {
        ...
        if (newCustomFieldsId) {
            ...
            return res.status(400).json({ status: 400, message: "custom fields not found for this id (" + id + ")" });
        }
    } catch (error) {
        console.log(error);
        return res.status(500).json({ status: 500, message: "failed to load custom details by id!" });
    }
}
...
const deleteCustomFields = async (req, res) => {
    ...
    try {
        ...
        if (deleteCustomFields) {
            return res.status(200).json({ status: 200, message: "delete custom fields this id (" + id + ")" });
        } else {
            return res.status(400).json({ status: 400, message: "not found this (" + id + ") id!" });
        }
    } catch (error) {
        console.log(error);
        return res.status(500).json({ status: 500, message: "failed to delete this (" + id + ") custom id!" });
    }
}
