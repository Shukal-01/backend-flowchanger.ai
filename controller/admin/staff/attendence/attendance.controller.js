    ...
    if (type === "day") {
      ...
      if (requestedDate > currentDate) {
        return res.status(400).json({
          message:
            "The requested date (${day}/${month}/${year}) is a future date. No entries are available for future dates.",
        });
      }
      ...
      if (eligibleStaff.length === 0) {
        return res.status(404).json({
          message:
            "No staff available for the requested date (${day}/${month}/${year}) as all have joining dates after this date.",
        });
      }
      ...
      for (const staff of eligibleStaff) {
        ...
        if (requestedDate < joiningDate) {
          return res.status(400).json({
            message:
              "The requested date (${day}/${month}/${year}) is before the joining date of staff ID ${id}. No entry will be created.",
          });
        }
      ...
      return res.status(200).json({
        message:
          "Punch records fetched successfully for the requested date (${day}/${month}/${year}).",
        records,
      });
    } else {
      return res
...
const getSingleStaffAttendance = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Attendance not fetched" + error.message });
  }
};
