...
const { PunchOutSchema, PunchRecordsSchema } = require('../../utils/validations');
...
async function createPunchIn(req, res) {
    try {
        ...
        PunchInSchema.parse({ staffId, punchInMethod, biometricData, qrCodeValue, photoUrl });
        ...
        const existingPunchIn = await prisma.punchIn.findFirst({
            where: {
                staffId,
                punchInDate: { // Check the punchInDate field
...
async function getPunchIn(req, res) {
    // try {
    //     const { id } = req.params;
    //     const punchIn = await prisma.punchIn.findMany({
    //         where: { id },
    //     });
    //     res.status(200).json(punchIn);
    // } catch (error) {
    //     console.log(error);
    ...
    try {
        ...
        console.log(error)
        return res.status(500).json({ error: 'Failed to retrieve punch-in records' });
    }
}
...
async function createPunchOut(req, res) {
    try {
        ...
        // Validate input using zod schema
        PunchOutSchema.parse({ staffId, punchOutMethod, biometricData, qrCodeValue, photoUrl });
        ...
        const existingPunchOut = await prisma.punchOut.findFirst({
            where: {
                staffId,
                punchOutDate: { // Check the punchInDate field
                    gte: today, // Greater than or equal to today
                    lt: new Date(today.getTime() + 86400000) // Less than tomorrow
                },
            },
...
async function createPunchRecords(req, res) {
    try {
        ...
        const existingPunch = await prisma.punchRecords.findFirst({
            where: {
                punchDate: { // Check the punchInDate field
                    gte: today, // Greater than or equal to today
                    lt: new Date(today.getTime() + 86400000) // Less than tomorrow
                },
                OR: [
        ...
        if (existingPunch) {
            return res.status(400).json({ message: "PunchInId or PunchOutId already exists for today." });
        }
...
module.exports = { createPunchIn, getPunchIn, createPunchOut, getPunchOut, createPunchRecords, getPunchRecords };